...
const tableName ="PST_TDE_TRANSACCION_DETALLES";
const tableId ="PST_TDE_ID";
...
exports.getDetalleAssignmentWithEmployee= async (req, res, next)=>{
    ...
    try {
        ...
        // let users = await getTest();
        let sql ={}
        sql.sqlList =  `SELECT PST_TDE_TRANSACCION_DETALLES.* FROM PST_TDE_TRANSACCION_DETALLES
        INNER JOIN PST_TRN_TRANSACCION ON PST_TRN_TRANSACCION.PST_TRN_ID = PST_TDE_TRANSACCION_DETALLES.PST_TDE_TRA_ID
        WHERE PST_TRN_TRANSACCION.PST_TRN_EMP_ID = ${req.query.EMPLOYEE_ID} AND
        PST_TRN_TRANSACCION.PST_TRN_TTR_ID = 1`;

        sql.sqlCount = `SELECT COUNT(*) FROM PST_TDE_TRANSACCION_DETALLES
        INNER JOIN PST_TRN_TRANSACCION ON PST_TRN_TRANSACCION.PST_TRN_ID = PST_TDE_TRANSACCION_DETALLES.PST_TDE_TRA_ID
        WHERE PST_TRN_TRANSACCION.PST_TRN_EMP_ID = ${req.query.EMPLOYEE_ID} AND
        PST_TRN_TRANSACCION.PST_TRN_TTR_ID = 1`;

        // let rows = await oracleController.getConsult(sql);
        let resp = await oracleController.getListBySql(tableName,sql);
        res.send(resp);
    } catch (error) {
        console.log({error:error});        
        res.status(500).send({error:error.message});
